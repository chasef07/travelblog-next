...
import { july2026Posts } from '@/content/blog-posts/2026-july'
...
export function getPostsForCountry(countryName: string): BlogPost[] {
  return allBlogPosts.filter((post) => post.country === countryName)
}

export function getArchivesForCountry(countryName: string): BlogArchive[] {
  const seen = new Set<string>()

  return getPostsForCountry(countryName)
    .map((post) => getArchiveForPost(post))
...
export function getPostCardImage(post: BlogPost): string {
  const primaryImage = post.images[0]?.src
  if (primaryImage) {
    return primaryImage
  }

  return getArchiveForPost(post)?.image || '/assets/images/misc/posttrip.jpg'
}

function getMonthSlugFromDate(date: string): string {
  ...
  return date.split(' ')[0]?.toLowerCase() ?? ''
}
